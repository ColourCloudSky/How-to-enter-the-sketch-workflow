...
## 六、团队协作
1、保持自己的symbols库和团队共享的symbols不重复。这样做的好处是文件合并的时候不会出现重复且没有更新的symbols组件。
...
4、在导出资源的时候，还是要针对每一个需要导出的元素设置切片范围，这样才可以正常导出。

5、设置切片时，需要将切片和被切片的元素放在一个组里面，再打开Export Group Contents Only,要不然就会出现带背景的情况。

6、导出小技巧。在用这个

![img](http://note.youdao.com/src/9835DC095D9B485386F3FFAB9D4C3C7F)

导出插件时，将自己需要导出的page放在最上面，这样可以方便自己找到需要导出的页面。

![img](http://note.youdao.com/src/4E1E8227D058478CBE4AA960FAF62538)

Tip:导出后，如果发现有问题，不要使用导出覆盖而是把之前有问题的文件夹全部删掉，重新导出一次。

7、每次迭代组件后，都需要检查下，是不是链接到迭代的共享组件，如果没有，需要单独调整。如果有在自己的组件库中发现与新更新的组件重复，则说明重复组件有更新过，需要重新选择新组件（老组件填的样式会保留，symbol没有嵌套的情况下），替换后删掉自己组件库中的重复组件。

![img](http://note.youdao.com/src/2C4B5975AE7848C6917D72438B1B0C73)

这样就发现有重复的组件。

8、命名规范：模块名——页面名字——控件名——状态。建议自己新建的模块symbols命名不要跟基础底层symbols命名规范一致。

9、文件更新时，会出现symbol属性丢失的情况，如何解决文件合并的问题？

1）symbols有重复且为嵌套symbol，则直接删除

![img](http://note.youdao.com/src/277897CB60BF4525A452B8848F5C0D25)

被创建的symbols页面中的重复symbol

2）symbols有重复且没有嵌套，则先选择更新的symbols，之后再删除被创建的symbols页面中的重复symbol
